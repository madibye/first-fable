...
func update_menu_state():
	match menu_state:
		MenuState.CommandMenu:
			...
			element_manager.set_targetable_elements(command_menu_elements)
		MenuState.FightMenu:
			...
			element_manager.set_targetable_elements(move_btns)
			
func disable_menu(menu: NinePatchRect):
	element_manager.disable_targetable_elements(menu.find_children("*", "", true).filter(func(e): return e in element_manager.targetable_elements))
	menu.hide()
...
func target_query(move: MoveBase):
	...
	while len(targets) < move.target_count:
		element_manager.set_targetable_elements(possible_targets.filter(func(e): return not e in targets))
		element_manager.element_selected.connect(add_target)
		await target_added
		if element_manager.element_selected.is_connected(add_target):
			element_manager.element_selected.disconnect(add_target)
	menu_state = MenuState.CommandMenu
	element_manager.disable_targetable_elements()
	await textbox_print("%s used %s!" % [current_turn.av_name, move.move_name])
	move.use_attack(targets)
